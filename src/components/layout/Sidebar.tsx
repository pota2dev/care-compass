...
import { useEffect, useState } from "react"; // Added for fetching data
...
export default function Sidebar() {
  ...
  useEffect(() => {
    const fetchCounts = async () => {
      try {
        const [notifRes, bookingRes] = await Promise.all([
        ...
        if (bookingRes.ok) {
          ...
          // Count only confirmed/upcoming bookings
          setActiveBookings(bookings.filter((b: any) => b.status === "CONFIRMED").length);
        }
      } catch (error) {
    ...
    fetchCounts();
  }, [pathname]); // Refresh counts when user navigates
  ...
  return (
    ...
    >
      {/* Logo */}
      <div className="p-5 border-b" style={{ borderColor: "rgba(45,80,22,0.1)" }}>
        <Link href="/" className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-lg flex items-center justify-center text-base" style={{ backgroundColor: "#2D5016" }}>
            🐾
          </div>
          <span className="font-display font-bold text-lg" style={{ color: "#2D5016" }}>
            CareCompass
          </span>
        </Link>
      </div>
      ...
      {user && (
        <div className="mx-3 mt-3 p-3 rounded-xl flex items-center gap-2.5" style={{ backgroundColor: "#FAF7F2" }}>
          ...
          <div className="min-w-0">
            ...
            <p className="text-[10px] truncate" style={{ color: "#8A9480" }}>{user.emailAddresses[0]?.emailAddress}</p>
          </div>
        </div>
      )}
            ...
            {section.items.map((item) => {
              ...
              return (
                <Link
                  ...
                  style={isActive ? { backgroundColor: "#C8DFB0", color: "#2D5016", fontWeight: 500 } : { color: "rgba(45,80,22,0.7)" }}
                >
      ...
      {/* Sign out */}
      <div className="p-3 border-t" style={{ borderColor: "rgba(45,80,22,0.1)" }}>
        <button onClick={() => signOut({ redirectUrl: "/" })} className="flex items-center gap-2.5 w-full px-3 py-2.5 rounded-xl text-sm transition-all" style={{ color: "rgba(45,80,22,0.7)" }}>
