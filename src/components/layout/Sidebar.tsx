...
import {
  ...
  CalendarCheck, Scissors, Home, Heart, AlertTriangle,
  User, Settings, LogOut, Sparkles, MessageCircle, Map,
} from "lucide-react";
...
const NAV_ITEMS = [
  ...
  },
  {
    label: "Tools",
    items: [
      { href: "/chat",     icon: MessageCircle, label: "Messages",            badge: "NEW" },
      { href: "/map",      icon: Map,           label: "Nearby Services",     badge: "NEW" },
      { href: "/pet-care", icon: Sparkles,      label: "Care Recommendations", badge: "AI" },
    ],
  },
  {
    label: "Account",
...
export default function Sidebar() {
  ...
  return (
    <aside className="hidden lg:flex w-60 flex-col bg-white border-r sticky top-0 h-screen"
      ...
      <div className="p-5 border-b" style={{ borderColor: "rgba(45,80,22,0.1)" }}>
        <Link href="/" className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-lg flex items-center justify-center text-base"
            style={{ backgroundColor: "#2D5016" }}>🐾</div>
      ...
      <nav className="flex-1 overflow-y-auto p-3 space-y-5 mt-2">
        {NAV_ITEMS.map((section) => (
          <div key={section.label}>
            <p className="px-3 mb-1.5 text-[10px] font-medium uppercase tracking-widest"
            ...
            {section.items.map((item) => {
              ...
              return (
                <Link key={item.href} href={item.href}
                  ...
                  style={isActive
                    ? { backgroundColor: "#C8DFB0", color: "#2D5016", fontWeight: 500 }
